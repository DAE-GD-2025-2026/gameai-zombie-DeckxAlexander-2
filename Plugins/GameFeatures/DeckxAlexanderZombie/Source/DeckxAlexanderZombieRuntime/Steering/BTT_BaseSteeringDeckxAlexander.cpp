...
UBTT_BaseSteeringDeckxAlexander::UBTT_BaseSteeringDeckxAlexander()
{
	bNotifyTick = true;
}

EBTNodeResult::Type UBTT_BaseSteeringDeckxAlexander::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
void UBTT_BaseSteeringDeckxAlexander::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
...
void UBTT_BaseSteeringDeckxAlexander::RotateTowardsMovement(APawn* Pawn, FVector movementdir, float DeltaTime)
{
	if (!Pawn) return;
...
FVector UBTT_BaseSteeringDeckxAlexander::CalculateObstacleAvoidance()
{
...
FVector UBTT_BaseSteeringDeckxAlexander::PurgeAvoidance()
{
...
FVector UBTT_BaseSteeringDeckxAlexander::CalculateAvoidanceInDirection(const FVector& position, const FVector& direction, float lookAheadDistance, float weight)
{
